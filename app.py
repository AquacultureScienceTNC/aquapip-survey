...
def render_survey():
    _persist_restore()
    _jump = False   # 'Back to Recommendations' is rendered beside step 2 below
    st.markdown("<div class='hero'><h1>Welcome to AquaPIP</h1>"
                "<p>The purpose of this survey is to help you match your interest in measuring "
                "certain co-benefits of your farm to verified, science-backed protocols. These "
                "protocols are aligned with The Nature Conservancy's Restorative Aquaculture MEL "
                "framework \u2014 a guide to a common and consistent approach to collecting and "
                "monitoring the environmental benefits generated by aquaculture.</p></div>",
                unsafe_allow_html=True)
    st.divider()
    ...
    # --- Step 2: what are you interested in measuring? (goal shortcuts + indicator chips) ---
    ...
    for area in ORDER:
        ...
        if not inds:
            ...
            continue
        labels = [it["label"] for it in inds]
        st.markdown(f"<div style='font-weight:700;color:{AREA_COLOR[area]};"
                    f"margin:.55rem 0 .1rem'>{area}</div>", unsafe_allow_html=True)
        if _use_pills:
            st.pills(area, labels, selection_mode="multi", key=GROUP_KEYS[area],
                     label_visibility="collapsed")
        else:
    ...
    codes = []
    for area in ORDER:
        sel = st.session_state.get(GROUP_KEYS[area], []) or []
        for it in ind_groups[area]:
            if it["label"] in sel and it["code"] not in codes:
...
def render_monitoring():
    ...
    step("Improve Global Understanding of Aquaculture")
    st.markdown("The Global Aquaculture Team is working to gain a better understanding of what "
                ...
                "responses will be saved in a database for TNC to inform global-scale decision "
                "making and improve tool delivery. No data will be shared externally, and it will "
                "be kept secure on a locked TNC database.")
...
def render_learn_indicators():
    ...
    st.markdown(f"<div class='hero'><h1>Your MEL indicators</h1>"
                f"<p>The full MEL indicator set for a <b>{aqua}</b> farm, laid out as in TNC's "
                f"Restorative Aquaculture MEL framework. This framework was carefully developed to "
                f"identify indicators that relate to co-benefits on your farm. The indicators you "
                f"selected are highlighted; the rest are greyed so you can see them in context.</p>"
    ...
    def matrix(area, inds):
        ...
        return (f"<div style='border:1px solid #E1E7E9;border-radius:10px;margin:.4rem 0 1.1rem'>"
                f"<div style='overflow-x:auto;transform:rotateX(180deg)'>"
                f"<div style='transform:rotateX(180deg)'>"
...
def _render_finalized(sub, matches, code2label):
    """Clean, filtered summary of the chosen protocol per indicator, with a sort
    control, per-indicator drop, PDF of the finalized set, and revise-back."""
    st.session_state["seen_results"] = True
    farm = sub.get("farm_name") or "Your farm"

    nav = st.columns([1.3, 1.5, 2.2])
    with nav[0]:
        if st.button("<-  Revise selection", use_container_width=True):
            st.session_state["finalized"] = False; st.rerun()
    with nav[1]:
        sort_choice = st.selectbox(
            "Sort by", ["Recommended order", "Cost (easiest first)",
                        "Effort (easiest first)", "Skill (easiest first)"],
            key="final_sort", label_visibility="collapsed")

    st.markdown(f"<div class='hero'><h1>Your finalized protocols</h1>"
                f"<p>{farm} \u00b7 {sub['aqua_type']} \u00b7 the protocols you've chosen, one per "
                f"indicator. Open any to see the full method, or drop the ones you don't want.</p>"
                f"</div>", unsafe_allow_html=True)

    dropped = st.session_state.setdefault("final_dropped", set())
    chosen = [(code, cands[0]) for code, cands in matches.items()
              if cands and code not in dropped]

    def _key(item):
        r = item[1]["row"]
        cost = r["cost_ord"] if r["cost_ord"] > 0 else 99
        eff = r["effort_ord"] if r["effort_ord"] > 0 else 99
        if sort_choice.startswith("Cost"):   return (cost, eff, -r["tier"])
        if sort_choice.startswith("Effort"): return (eff, cost, -r["tier"])
        if sort_choice.startswith("Skill"):  return (-r["tier"], eff, cost)
        return (0,)   # recommended: keep the ranked order
    if not sort_choice.startswith("Recommended"):
    ...
    st.divider()
    final_matches = {code: [c] for code, c in chosen}
    try:
        pdf = report.build_pdf(sub, final_matches, code2label, name_map=CODE_NAMES)
        fname = (sub.get("farm_name") or "MEL").replace(" ", "_")[:40]
        st.download_button("Download PDF of finalized protocols", data=pdf,
                           file_name=f"{fname}_MEL_finalized.pdf",
...
def render_results():
    ...
    # ranked matches + user overrides (shared by the recommendations and finalized views)
    sel = sub.get("mon_sel", [])
    ...
    if st.session_state.get("finalized"):
        _render_finalized(sub, matches, code2label); return
    ...
    fcol = st.columns([1.7, 3])
    with fcol[0]:
        if st.button("\u2713  Finalize my selection  \u2192", type="primary", use_container_width=True):
            st.session_state["finalized"] = True; st.rerun()
    with fcol[1]:
        st.caption("Happy with your picks? Finalize to lock them in and get a clean, downloadable "
                   "summary \u2014 you can revise anytime.")
...
def _detail(r, covers, keyns="", stacked=False, show_title=False):
    if show_title and r.get("display_title"):
        st.markdown(f"**Protocol:** {r['display_title']}")
    st.markdown(f"**MEL Indicator:** {' \u00b7 '.join(covers)}")
